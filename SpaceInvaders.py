...
def game_over(player, enemies, writer):
  for enemy in enemies:
    if (abs(player.player.xcor() - enemy.enemy.xcor()) < 10 and abs(player.player.ycor() - enemy.enemy.ycor()) < 15):
...
def main():
  ...
  controller = s.Serial(port='COM12', baudrate=9600, timeout=0)

  try:
    while True:
      if controller.in_waiting > 0:
        try:
          data = controller.readlines()[-1].decode('utf-8', errors='ignore').strip()
          parts = data.split(",")
          if len(parts) == 2:
            x = int(parts[0])
            p = int(parts[1])
            screen_x = (x / 1022) * 550 - 275
            player.player.setposition(screen_x, -250)
            if p == 0:
              player.fire_bullet()
          move_enemy(enemies)
          player.bullet_move()
